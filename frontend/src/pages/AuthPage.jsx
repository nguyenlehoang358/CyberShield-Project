...
function PasswordStrengthMeter({ password, lang, t }) {
    ...
    const checkItems = [
        { ok: analysis.checks.length >= 8, label: t('pwd_check_length').replace('{n}', analysis.checks.length) },
        { ok: analysis.checks.hasLower, label: t('pwd_check_lower') },
        { ok: analysis.checks.hasUpper, label: t('pwd_check_upper') },
        { ok: analysis.checks.hasDigit, label: t('pwd_check_digit') },
        { ok: analysis.checks.hasSpecial, label: t('pwd_check_special') },
        { ok: !analysis.isCommon, bad: analysis.isCommon, label: t('pwd_check_common') },
    ]

    const warningLabels = {
        numeric: t('pwd_warn_numeric'),
        alpha: t('pwd_warn_alpha'),
        repeat: t('pwd_warn_repeat'),
    }
        ...
        <div className="password-strength-meter">
            ...
            <div className="strength-header">
                ...
                <span className="strength-value" style={{ color: analysis.strengthColor }}>
                    {analysis.score}% — {t(strengthKeys[analysis.strength])}
                </span>
            </div>
